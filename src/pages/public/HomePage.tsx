...
const whyChooseUs = [
  {
    icon: ShieldCheck,
    title: 'Trusted Across Enugu',
    desc: 'Hundreds of households and businesses trust us with their laundry every week.',
  },
  {
    icon: Clock,
    title: 'Live Order Tracking',
    desc: 'Know exactly where your order is, from pickup to a freshly pressed finish.',
  },
  {
    icon: BadgeCheck,
    title: 'Transparent Pricing',
    desc: 'No hidden fees. See the exact price per item before you place an order.',
  },
  {
    icon: Leaf,
    title: 'Eco-Conscious Wash',
    desc: 'Detergents and processes chosen to be gentle on fabric and on the environment.',
  },
];
...
const trackingSteps = [
  {
    ...
    desc: 'Agent assigned and on the way',
    status: 'done' as const,
  },
  {
    title: 'In Processing',
    desc: 'Your clothes are undergoing specialized care',
    status: 'active' as const,
  },
  {
    title: 'Ready for Delivery',
    desc: 'Final quality inspection completed',
...
const testimonials = [
  {
    name: 'Ngozi A.',
    quote:
      'Express turnaround saved me before a big meeting. Spotless shirts, right on time.',
    rating: 5,
  },
  {
    name: 'Chukwuemeka O.',
    quote:
      'The tracking page is genuinely useful, I always know exactly where my order is.',
    rating: 5,
  },
  {
    name: 'Adaeze I.',
    quote: 'My agbada came back looking brand new. Worth every naira.',
    rating: 5,
  },
];
  ...
  return (
    <div className='flex flex-col'>
      ...
      <section className='relative flex min-h-105 items-center overflow-hidden md:min-h-140'>
        ...
        <div className='relative mx-auto flex w-full min-w-0 max-w-shell flex-col gap-stack-sm px-margin-mobile py-16 md:px-gutter md:py-24'>
          ...
          <p className='max-w-sm text-body-md text-clean-white/85 md:max-w-md md:text-body-lg'>
            Clean wash, iron, pickup and delivery for Enugu, track every stage
            live.
          </p>
          ...
          <div className='mt-8 flex min-w-0 items-center gap-3 md:gap-8'>
            {heroStats.map((stat, i) => (
              <div
                key={stat.label}
                className={cn(
                  'flex min-w-0 items-center gap-3 md:gap-8',
                  i > 0 && 'border-l border-clean-white/20 pl-3 md:pl-8',
                )}
              >
                <div className='min-w-0'>
                  <p className='font-display text-label-md text-clean-white md:text-headline-md'>
                    {stat.value}
                  </p>
                  <p className='whitespace-nowrap text-[10px] leading-tight text-clean-white/70 md:text-label-sm'>
                    {stat.label}
                  </p>
                </div>
              </div>
              ...
              {banner.image_url ? (
                <>
                  ...
                  <div className='relative flex w-full items-center justify-between p-stack-md'>
                    <div>
                      <p className='text-label-sm font-bold uppercase text-clean-white'>
                        {banner.title}
                      </p>
                      <p className='text-headline-md font-display text-clean-white'>
                        {banner.subtitle}
                      </p>
                    </div>
                  </div>
                ...
                <div className='flex w-full items-center justify-between bg-linear-to-r from-secondary-fixed to-secondary-container p-stack-md'>
                  <div>
                    <p className='text-label-sm font-bold uppercase text-on-secondary-container'>
                      {banner.title}
                    </p>
                    <p className='text-headline-md font-display text-on-secondary-container'>
                      {banner.subtitle}
                    </p>
                  </div>
      ...
      <div className='mx-auto w-full min-w-0 max-w-shell px-margin-mobile md:px-gutter'>
        ...
        <div className='mt-stack-lg grid grid-cols-1 gap-stack-md md:grid-cols-3'>
          {valueProps.map((v) =>
            v.variant === 'featured' ? (
              <div
                key={v.title}
                className='flex flex-col gap-stack-sm rounded-2xl bg-laundry-blue-deep p-stack-lg shadow-soft-lift md:scale-105'
              >
                ...
                <p className='text-body-lg font-bold normal-case text-clean-white'>
                  {v.title}
                </p>
              ...
              <div
                key={v.title}
                className='flex flex-col gap-stack-sm rounded-2xl bg-surface-container-low p-stack-lg'
              >
                <div className='flex h-11 w-11 items-center justify-center rounded-xl bg-primary/10'>
                  <v.icon className='h-5 w-5 text-primary' />
                </div>
                <p className='text-body-lg font-bold normal-case text-on-surface'>
                  {v.title}
                </p>
                <ul className='flex flex-col gap-1.5'>
                  {v.bullets.map((b) => (
                    ...
                    >
                      <Check className='h-4 w-4 shrink-0 text-success-green' />{' '}
                      {b}
                    </li>
                  ))}
                </ul>
        ...
        <div className='mt-stack-lg grid grid-cols-2 gap-3'>
          <Link to={paths.trackLookup} className='block w-full'>
            <Card className='w-full pt-stack-sm transition-shadow hover:shadow-soft-lift'>
              <CardContent className='flex w-full flex-col items-center justify-center gap-3 py-stack-md text-center'>
                <div className='flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-primary/10'>
                  <Package className='h-5 w-5 text-primary' />
                </div>
                <span className='text-label-md font-bold normal-case text-on-surface'>
                  Track Order
                </span>
              </CardContent>
            </Card>
      ...
      <section className='mt-stack-lg bg-surface-container-low py-stack-lg md:py-16'>
        <div className='mx-auto w-full min-w-0 max-w-shell px-margin-mobile md:px-gutter'>
          <h2 className='text-center text-headline-md font-display text-on-surface md:text-headline-lg'>
            Why Choose Us
          </h2>
      ...
      <section className='py-stack-lg md:py-16'>
        <div className='mx-auto w-full min-w-0 max-w-shell px-margin-mobile md:px-gutter'>
          <div className='grid grid-cols-1 items-center gap-stack-lg md:grid-cols-2 md:gap-stack-lg'>
            ...
            />
            <div>
              <h2 className='text-headline-md font-display text-on-surface md:text-headline-lg'>
                Track Every Step
              </h2>
              <p className='mt-stack-sm text-body-md text-on-surface-variant md:text-body-lg'>
                Stay informed from the moment we pick up your laundry until it
                arrives back at your doorstep, fresh and pressed.
              </p>
                ...
                {trackingSteps.map((step) => (
                  <div key={step.title} className='flex items-start gap-3'>
                    ...
                    <div>
                      <p
                        className={cn(
                          'text-label-md font-bold normal-case',
                          step.status === 'pending'
                ...
                <CardContent className='flex flex-col gap-2 pt-stack-md'>
                  <div className='flex gap-0.5'>
                    {Array.from({ length: t.rating }).map((_, i) => (
                      <Star
                        key={i}
                        className='h-4 w-4 fill-secondary-container text-secondary-container'
                      />
                    ))}
                  </div>
                  <p className='text-body-md text-on-surface-variant'>
      ...
      <section className='bg-primary py-stack-lg md:py-16'>
        <div className='mx-auto flex w-full min-w-0 max-w-shell flex-col items-center gap-stack-md px-margin-mobile text-center md:px-gutter'>
          <h2 className='text-headline-md font-display text-on-primary md:text-headline-lg'>
            Ready for fresher laundry?
          </h2>
          <p className='max-w-md text-body-md text-on-primary/90'>
            Build your order in minutes and let us handle the rest: wash, iron,
            pickup, delivery.
          </p>
